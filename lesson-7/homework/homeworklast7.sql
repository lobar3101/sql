...
Write a query to use HAVING to filter departments having more than 5 employees from the Employees table.
(DeptID is enough, if you dont have DeptName)
...
SELECT sum(saleamount),avg(saleamount),ProductID FROM Sales
group by ProductID
SELECT count(*), count(employeeId), DepartmentName FROM Employees
where DepartmentName='HR'
group by DepartmentName
Write a query that finds the highest (MAX) and lowest (MIN) Salary by department in the Employees table.
(DeptID is enough, if you dont have DeptName).
select max(salary), min(salary), departmentname from Employees
group by DepartmentName

Write a query that uses GROUP BY to calculate the average salary per Department.
(DeptID is enough, if you dont have DeptName).
select avg(salary), DepartmentName from Employees
GROUP BY DepartmentName



select AVG (salary) as avgsalary,COUNT(*)as empnumber, departmentname from employees 
GROUP BY departmentname
...
SELECT SUM(SALEAMOUNT) AS TOTALSALEAMOUNT,SaleDate FROM Sales
GROUP BY SaleDate
...
HAVING 
    sum(Salary) >500;
Write a query that shows the average (AVG) sales for each product category, 
and then uses HAVING to filter categories with an average sales amount greater than 200.
select avg(saleamount),ProductID from Sales
group by ProductID
...
select sum(salary) as sumsal, avg(salary) as avgsal,DepartmentName from employees
group by DepartmentName
having avg(salary)>65000

Write a query that finds the maximum (MAX) and minimum (MIN) order value for each customer, 
...
select max(orderID),min(orderID),CustomerID from Orders
group by CustomerID
