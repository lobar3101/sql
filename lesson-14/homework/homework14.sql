select *, name  from TestMultipleColumns
SELECT
    TRIM(LEFT(Name, CHARINDEX(',', Name + ',') - 1)) AS LastName,
    TRIM(SUBSTRING(Name, CHARINDEX(',', Name) + 1, LEN(Name))) AS FirstName
FROM TestMultipleColumns;
Write a SQL query to find strings from a table where the string itself contains 
the % character.(TestPercent)
select * from TestPercent
SELECT *
FROM TestPercent
WHERE Strs LIKE '%\%%' ESCAPE '\';

In this puzzle you will have to split a string based on dot(.).()
select*from Splitter
SELECT 
   
    LEFT(vals, CHARINDEX('.', vals + '.') - 1) AS part_before_first_dot,
    STUFF(vals, 1, CHARINDEX('.', vals + '.'), '') AS part_after_first_dot
FROM Splitter
...
Find the employees who have been with 
the company for more than 10 years, but less than 15 years. 
...
FROM 
    Employees
WHERE 
    HIRE_DATE BETWEEN DATEADD(YEAR, -15, GETDATE()) 
                AND DATEADD(YEAR, -10, GETDATE());

Write a SQL query to separate the integer values and the character values into two different columns.
(rtcfvty34redt)

SELECT 
    TRANSLATE('rtcfvty34redt', 'abcdefghijklmnopqrstuvwxyz', SPACE(26)) AS numbers,
    TRANSLATE('rtcfvty34redt', '0123456789', SPACE(10)) AS letters;
...
FROM 
	...
	select * from fruits
SELECT 
    (substring(fruit_list, ','))[3] AS third_fruit
FROM fruits
...
WHILE @i <= LEN(@str)
BEGIN
    INSERT INTO character_table (char)
...
JOIN 
    ...
    CASE
        ...
        WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, hire_date)) BETWEEN 1 AND 4 THEN 'Junior'
        WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, hire_date)) BETWEEN 5 AND 9 THEN 'Mid-Level'
        WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, hire_date)) BETWEEN 10 AND 20 THEN 'Senior'
        ELSE 'Veteran'
    ...
    CASE 
        WHEN Vals LIKE '[0-9]%' THEN 
            CAST(SUBSTRING(Vals, PATINDEX('%[0-9]%', Vals), 
                 PATINDEX('%[^0-9]%', Vals + ' ') - PATINDEX('%[0-9]%', Vals)) AS INT)
        ELSE NULL
...
FROM 
	...
	SELECT 
    Id,
    Vals AS original_string,
    CONCAT(
        SUBSTRING(SUBSTRING(Vals, CHARINDEX(',', Vals) + 1, 1), 1, 1),
        ',',
        SUBSTRING(Vals, 1, 1),
        SUBSTRING(Vals, CHARINDEX(',', Vals) + 2, LEN(Vals))
    ) AS swapped_string
FROM 
    MultipleVals;
	SELECT DISTINCT
...
WeeklyTotals AS (
    ...
    FROM
        CombinedSales
    GROUP BY
        Area, FinancialYear, FinancialWeek
)
SELECT
    c.Area,
    c.FinancialYear,
    ...
    c.DayName,
    c.DayOfWeek,
    c.TotalSales AS DailySales,
    w.WeeklyTotal,
    CASE
        WHEN w.WeeklyTotal = 0 THEN 0
        ELSE ROUND((c.TotalSales * 100.0 / w.WeeklyTotal), 2)
    END AS PercentageOfWeeklySales
FROM
    CombinedSales c
JOIN
    WeeklyTotals w ON c.Area = w.Area 
                   AND c.FinancialYear = w.FinancialYear
                   AND c.FinancialWeek = w.FinancialWeek
ORDER BY
    c.Area, c.FinancialYear, c.FinancialWeek, c.DayOfWeek;