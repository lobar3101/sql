...
select p.ProductID,s.SupplierID, p.Productname, s.Suppliername from Suppliers s
...
select e.DepartmentID  from Employees e
...
select * from orders
select o.OrderID, c.firstname, c.lastname from Customers c
...
select s.StudentID, s.name, s.Major as coursenamee, c.	CourseID, c.CourseName  from Students s
cross join Courses c
select * from Courses
Using Products, Orders table Get product names and orders where product IDs match.
select p.Productname, o.orderID,o.ProductID from Products p
...
select d.DepartmentID, d.departmentName, e.name, e.salary from Departments d
...
select s.StudentID, e.CourseID, e.name, e.major  from Students s
...
CREATE TABLE Payments ( PaymentID INT PRIMARY KEY, OrderID INT NULL, PaymentDate DATE, Amount DECIMAL(10, 2), PaymentMethod VARCHAR(50), FOREIGN KEY (OrderID) REFERENCES Orders(OrderID) );
 ...
 select p.PaymentID, p.OrderID, p.PaymentDate, p.Amount, p.PaymentMethod, o.totalamount, o.ProductID 
 from Payments p
 inner join Orders o on p.OrderID=o.OrderID
 inner join Payments  on p.Amount=o.TotalAmount
Using Orders, Products table Show orders where product price is more than 100.

select * from Products
select o.productID, o.orderID,o.totalamount, p.productname, p.price,p.category from Orders o
...
select e.EmployeeID, e.DepartmentID,e.salary, d.DepartmentName  from Employees e
...
select * from Orders
select * from Products
SELECT o.order_id, o.product_id, o.ordered_quantity, p.stock_quantity
FROM Orders o
JOIN Products p ON o.product_id = p.product_id
WHERE o.ordered_quantity > p.stock_quantity;
select c.FirstName, c.LastName,c.CustomerID, s.SaleID,s.SaleAmount  from Sales s
...
select * from Courses
select * from Students
SELECT s.studentID, c.coursename, s.name
FROM Enrollments e
...
select b.* , e.Name as [managername] from Employees e
join Employees b ON e.managerid = b.EmployeeID
where b.salary > e.managerid

Using Products, Categories table Show products where category is either 'Electronics' or 'Furniture'.
select * from Products
select * from Categories
select p.ProductName,c.categoryname, p.category, c.categoryID, p.price from Categories c
join Products p on p.Category=c.CategoryName
