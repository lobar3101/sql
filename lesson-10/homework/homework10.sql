...
CREATE TABLE Products ( ProductID INT PRIMARY KEY, ProductName VARCHAR(100), Price DECIMAL(10, 2), Category VARCHAR(50), StockQuantity INT, SupplierID INT );
...
select * from Sales
 ...
 select s.SaleAmount, s.ProductID,p.price,p.productname from Sales s
 join Products p on s.ProductID=p.ProductID
 where p.ProductID is null

 select e.employeeID, e.name, e.departmentID, d.departmentname from Employees e
  join 
select * from Departments
 * from 
SELECT * from Employees
...
FULL OUTER JOIN 
	...
	**Using the `Employees` table**, write a query to
	show each employees name along with the name of their manager.  
	select e.name, m.managerID, e.Salary,e.EmployeeID  from Employees e
	left join Employees m on m.ManagerID = e.EmployeeID
...
*Using the `Students`, `Courses`, and `Enrollments` tables**, 
write a query to list the names of students who are enrolled in the course named 'Math 101'.  
select s.StudentID, s.name, e.StudentID, e.CourseID, c.courseid,e.EnrollmentID,  c.coursename from Students s
...
WHERE 
   ...
   select * from Orders
   select c.customerid, c.firstname, c.lastname, o.quantity, o.TotalAmount, o.orderid from Customers c
...
9. **Using the `Employees` and `Departments` tables**,
write a query to list employees working in the 'Human Resources' department. 
select e.name, d.departmentname, e.departmentid from Employees e
join Departments d on  e.departmentid = d.DepartmentID
WHERE 
    d.departmentname = 'Human Resources'
	**Using the `Employees` and `Departments` tables**, 
	write a query to return department names that have more than 10 employees.  
select * from Employees
select d.DepartmentID,d.departmentname, e.employeeid from Departments d
join Employees e on d.DepartmentID= e.DepartmentID
GROUP BY 
    d.departmentid, d.departmentname,e.employeeid
HAVING 
...
14. **Using the `Employees` table**, write a query to find pairs of employees 
who report to the same manager.  
   🔁 *Expected Output:* `Employee1`, `Employee2`, `ManagerID`
   select a.name AS Employee1,
    b.name AS Employee2,
	b.ManagerID from Employees a
	join Employees b ON a.managerid = b.managerid

   

15. **Using the `Orders` and `Customers` tables**, write a query to list all 
orders placed in 2022 along with the customer name.    
...
WHERE 
	 ...
	 **Using the `Employees` and `Departments` tables**, write a query to 
...
FROM 
    Products p
LEFT JOIN 
    Categories c ON p.Category = c.categoryname
WHERE 
...
WHERE 
	 ...
	 **Using the `Products` and `Sales` tables**, write a query to list sales 
...
25. **Using the `Students`, `Courses`, and `Enrollments` tables**, write a query to find 
...
26. **Using the `Orders` and `Payments` tables**, write a query to return orders
that are missing payment details.  
...
27. **Using the `Products` and `Categories` tables**, write a query to list products that
belong to either the 'Electronics' or 'Furniture' category.  
...
FROM 
    Products p
JOIN 
    Categories c ON c.categoryname = p.category
WHERE 
