...
select P.Category,P.productName,p.productID,
S.SupplierID,S.supplierName,
 ISNULL(P.ProductName, 'No Product Supplied') AS PName
from Products p
...
SELECT * FROM Employees
SELECT a.*, b.Name as [ManagerName] FROM Employees a
JOIN Employees b
...
select * from Products
select * from Sales
SELECT P.ProductID,P.ProductName,P.Price, S.saleamount,S.SaleID,S.customerID
FROM Products P
LEFT JOIN Sales S
...
HAVING 
	...
	SELECT * FROM Employees 
	SELECT * FROM Departments
SELECT e.EmployeeID,e.name,d.DepartmentID, d.DepartmentID
FROM Employees e
...
select *from Orders
select *from Customers
select o.orderID, o.OrderDate,o.ProductID, c.FirstName, c.LastName
from Orders o
...
select o.OrderID, o.ProductID, p.PaymentID
FROM Orders o
full join Payments p
...
select * from Orders
select * from Payments
SELECT o.OrderDate, p.PaymentDate, p.PaymentID,
o.OrderID,  
    p.Amount
FROM Orders O
...
select p.ProductName, p.ProductID,c.CategoryID
from Products p
...
FROM 
    Students s
LEFT OUTER JOIN 
    Courses c ON s.StudentID = e.StudentID
...
select * from Customers
select * from Orders
select c.firstname, c.lastname, c.city, c.country,o.totalamount,o.productID,o.OrderID,o.Quantity
FROM Customers c
inner join Orders o 
on c.CustomerID=o.CustomerID
WHERE o.Quantity>3
...
select e.name,e.EmployeeID,D.DepartmentName
from Employees e
left join Departments d
on e.DepartmentID=d.DepartmentID
